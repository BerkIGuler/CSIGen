...
radio_map_samples_per_tx: 100000000
...
path_solver_max_num_paths_per_src: 1000000
path_solver_samples_per_src: 1000000
...
\subsubsection{Ray Tracing Parameters}
...
\begin{itemize}
  \item \texttt{max\_depth}: Maximum ray-scene interactions per path.
  \item \texttt{specular\_reflection}: Mirror-like reflections.
  \item \texttt{diffuse\_reflection}: Scattering from rough surfaces.
  \item \texttt{refraction}: Transmission through materials (Snell's law).
  \item \texttt{diffraction}: Signal bending around obstacles.
  \item \texttt{edge\_diffraction}: Diffraction at edges.
...
\begin{itemize}
  ...
  \item \textbf{Radio Map Pre-filtering}: Uses fewer samples than detailed path solving for efficient coverage estimation.
\end{itemize}
