...
# Scene configuration
scene_xml_path: "../scenes/boston_1/scene.xml"  # saved via scenegen command
carrier_frequency: 3500000000  # in Hz

# Scene setup parameters
scene_center: [0.0, 0.0]  # in meters
antenna_height_offset: 10.0  # in meters
num_deployment_buildings: 1
clip_terrain_to_buildings: true
terrain_clip_margin: 15.0  # in meters
user_shift_from_ground: 1.5  # in meters

# TX antenna array parameters
tx_num_rows: 2
tx_num_cols: 2
tx_vertical_spacing: 0.5  # in units of carrier wavelength
tx_horizontal_spacing: 0.5  # in units of carrier wavelength
tx_pattern: "tr38901"
...
rx_num_rows: 1
rx_num_cols: 1
rx_vertical_spacing: 0.5  # in units of carrier wavelength
rx_horizontal_spacing: 0.5  # in units of carrier wavelength
rx_pattern: "hw_dipole"
rx_polarization: "VH"

# Base station parameters
num_sectors: 3
mechanical_tilt: 10.0  # in degrees
azimuth_offset: 0.0  # in degrees
tx_power_dbm: 43.0  # in dBm

# Radio map solver parameters
radio_map_specular_reflection: true
radio_map_diffuse_reflection: true
radio_map_refraction: true
radio_map_diffraction: true
radio_map_edge_diffraction: true
radio_map_diffraction_lit_region: false
radio_map_max_depth: 5
radio_map_samples_per_tx: 1000000  # 10^6
radio_map_seed: 1

# User sampling parameters
num_user_samples_per_tx: 1000
user_sample_seed: 1
user_sample_min_val_db: -150  # in dB
user_sample_max_val_db: 0  # in dB
user_sample_min_dist: 0.0  # in meters
user_sample_max_dist: 10000.0  # in meters
user_sample_metric: "path_gain"
tx_association: true
sample_center_pos: true
scene_edge_epsilon: 1.0  # in meters (0.0 to disable edge filtering)

# Mobility preset
mobility_preset: "stationary"

# User equipment mobility presets
mobility_presets:
  stationary:
    orientation_mode: "random"
    speed_distribution: null
  stationary_to_tx:
...
path_solver_max_depth: 5
path_solver_max_num_paths_per_src: 10000000  # 10^7
path_solver_samples_per_src: 10000000  # 10^7
path_solver_synthetic_array: true
path_solver_los_mode: true
path_solver_specular_reflection: true
path_solver_diffuse_reflection: true
path_solver_refraction: true
path_solver_diffraction: true
path_solver_edge_diffraction: true
path_solver_diffraction_lit_region: false
path_solver_seed: 1
path_solver_per_tx_users_only: true
...
\begin{itemize}
  \item \texttt{num\_rows}, \texttt{num\_cols}: Array dimensions.
  \item \texttt{vertical\_spacing}, \texttt{horizontal\_spacing}: Element spacing in wavelengths.
  \item \texttt{pattern}: Radiation pattern---\texttt{tr38901} (3GPP), \texttt{dipole}, \texttt{iso}, or \texttt{hw\_dipole}.
  \item \texttt{polarization}: \texttt{cross} ($\pm 45^{\circ}$), \texttt{V}, \texttt{H}, or \texttt{VH}.
\end{itemize}
...
These control ray-scene interactions for both radio map and path solver. See \href{https://arxiv.org/pdf/2504.21719}{Sionna RT: Technical Report} for details.
\begin{itemize}
  ...
  \item \texttt{specular\_reflection}, \texttt{diffuse\_reflection}: Mirror-like and rough-surface scattering.
  \item \texttt{refraction}: Transmission through materials (Snell's law).
  \item \texttt{diffraction}, \texttt{edge\_diffraction}: Signal bending and edge diffraction.
  \item \texttt{diffraction\_lit\_region}: Diffraction in LoS regions.
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{tx\_association}: Associate users with strongest-signal TX.
  \item \texttt{sample\_center\_pos}: Sample from cell centers (deterministic) vs. randomly within cells.
...
\begin{itemize}
  ...
  \item \texttt{direction\_mode}: Movement direction (\texttt{random}).
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{cfr\_out\_type}: Output backend (numpy, TensorFlow, PyTorch, JAX, Dr.Jit).
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{CFR Computation}: Convert paths to frequency response arrays.
  \item \textbf{Valid-Path Filtering}: Only receivers with at least one valid path (delay $> 0$) are kept; channels with no valid paths are excluded from the output.
\end{enumerate}
...
All parameters are validated before generation, catching errors early with clear messages.
...
\subsection{Valid-Path Filtering}

After path solving and CFR computation, the pipeline keeps only channels (TX--RX pairs) for which at least one path has non-zero delay. Receivers with no valid paths are dropped, and the saved CFR arrays and metadata (e.g., \texttt{rx\_positions}, \texttt{rx\_names}) are restricted to this subset. The metadata field \texttt{num\_valid\_channels} reports the count of valid channels per TX. We include a histogram of number of paths from a TX to all sampled users displayed in Fig.~\ref{fig:num_paths}.
